...
private
  #Gets a random tweet from a users twitter timeline
  #user - A string of the user's twitter handle, sans @
  def get_random_tweet(user)
    tweets = Twitter.user_timeline(user, language:'en')
    tweets.sample
  end
  
  #Finds all words in a tweet, removing links, hashtags, and other twitter handles
  ...
  # Removes all common "stop" words that may prevent the Giphy API from finding a relevant GIF
  ...
  # Attempts different combinations of tweet keywords until a tweet can be found for a combination of keywords.
  ...
  # Chooses two random keywords from all eligible keywords
  def chosen_keywords(tweet_keywords)
    @keywords = tweet_keywords.sample(2)
  end

  # Obtains url of specific gif to be displayed
  ...
  #Creates logic for building a giphy search url
